...
Note : Notre table `session` s'appelle `session` car `session` est un mot-clé. Pour utiliser les requêtes suivantes, il faudra donc peut-être changer le nom de cette table
